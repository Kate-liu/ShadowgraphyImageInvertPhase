...
if test <= 1
    ...
    figure(1);
    subplot(2,2,1);
    imagesc(sourceMap); colormap gray;
    title('sourceMap');
    
    subplot(2,2,2);
    imagesc(Phi); colormap default; colorbar;
    title('Phi£ºDeflection potential');
    
    subplot(2,2,3);
    imagesc(targetMap); colormap gray;
    title('targetMap£ºShadowgram image');
    
    subplot(2,2,4);
    plot(targetMap(ceil(end/2),:));
    title('targetMap£ºCentral horizontal slice');
end
if test == 1
    ...
    title('PhiI£ºRetrieved deflection potential');
    ...
    title('Phi(blue) and Phil(green)£ºHorizontal slice of the deflection potentials');
